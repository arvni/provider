...
const loginFormValidator = (data, onerror) => {
  let output = true;
...
const changePasswordValidator = (data, onerror, user) => {
  let output = true;
...
const forgetPasswordValidator = (data, onerror) => {
  let output = true;
...
const resetPasswordValidator = (data, onerror) => {
  let output = true;
  if (!validator.isEmail(data.email) || validator.isEmpty(data.email)) {
    output = false;
...
const storeOrderMaterialValidator = (data, onError) => {
  let output = true;
...
const storeSampleTypeValidator = (data, onError) => {
  let output = true;
...
const storeSymptomGroupValidator = (data, onError) => {
  let output = true;
  if (validator.isEmpty(data.name)) {
    onError("name", "Please Enter Name");
    output = false;
...
const storeSymptomValidator = (data, onError) => {
  let output = true;
  if (validator.isEmpty(data.name)) {
    onError("name", "Please Enter Name");
    output = false;
...
const storeConsentGroupValidator = (data, onError) => {
  let output = true;
...
const storeDiseaseGroupValidator = (data, onError) => {
  let output = true;
  if (validator.isEmpty(data.name)) {
    onError("name", "Please Enter Name");
    output = false;
...
const storeTestValidator = (data, onError) => {
  let output = true;
...
const patientDetailsValidate = (data, onError) => {
  let output = true;
  if ((data == null ? void 0 : data.consanguineousParents) === null) {
    output = false;
    onError("consanguineousParents", "Please that parents are consanguineous ");
  }
  if (validator.isEmpty((data == null ? void 0 : data.fullName) ?? "")) {
    output = false;
...
const testMethodValidate = (data, onError) => {
  let output = true;
...
export {
  storeDiseaseGroupValidator as a,
  storeSampleTypeValidator as b,
  storeSymptomValidator as c,
  storeSymptomGroupValidator as d,
  storeTestValidator as e,
  forgetPasswordValidator as f,
  changePasswordValidator as g,
  storeOrderMaterialValidator as h,
  loginFormValidator as l,
  patientDetailsValidate as p,
  resetPasswordValidator as r,
  storeConsentGroupValidator as s,
  testMethodValidate as t
};
